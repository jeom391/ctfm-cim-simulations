...
export interface Capabilities { engines:Record<string,Availability>; effects?:Record<string,Availability|boolean>; hardware?:{tile_sizes?:number[];adc_bits?:number[];validated_combinations?:{tile_size:number;adc_bits:number;engine?:string;range_policy?:string}[]}; models?:unknown; [key:string]:unknown }
...
export interface SimulationForm {profileKeys:string[];pools:string[];mappings:string[];d2d:boolean;retention:boolean;adc:boolean;arrays:number;years:string;seed:number;tileSize:number;adcBits:number;engine:string;checkpoint:string}
